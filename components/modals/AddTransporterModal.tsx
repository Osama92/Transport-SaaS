...
interface AddTransporterModalProps {
    onClose: () => void;
    onAdd: (transporter: Omit<Transporter, 'id' | 'status'>) => void;
}
...
const AddTransporterModal: React.FC<AddTransporterModalProps> = ({ onClose, onAdd }) => {
    ...
    return (
        <ModalBase title={t('modals.addTransporter.title')} onClose={onClose}>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end gap-3 pt-4 border-t mt-6 dark:border-slate-700">
                    ...
                    <button type="submit" className="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-2 px-4 rounded-lg dark:bg-indigo-600 dark:hover:bg-indigo-700">{t('modals.addTransporter.saveButton')}</button>
                </div>
            </form>
        </ModalBase>
    );
