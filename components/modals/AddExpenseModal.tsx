...
const AddExpenseModal: React.FC<AddExpenseModalProps> = ({ onClose, route, onSave }) => {
    ...
    return (
        <ModalBase title={t('modals.addExpense.title', { routeId: route.id })} onClose={onClose}>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     <div>
                        <label htmlFor="type" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('modals.addExpense.type')}</label>
                        <select id="type" name="type" value={expense.type} onChange={handleChange} className="w-full px-3 py-2 rounded-lg bg-gray-100 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:bg-slate-700 dark:border-slate-600 dark:text-gray-200">
                            <option value="Fuel">{t('modals.addExpense.types.fuel')}</option>
                            <option value="Tolls">{t('modals.addExpense.types.tolls')}</option>
                            <option value="Maintenance">{t('modals.addExpense.types.maintenance')}</option>
                            <option value="Other">{t('modals.addExpense.types.other')}</option>
                        </select>
                    </div>
                    <InputField label={t('common.date')} id="date" name="date" type="date" value={expense.date} onChange={handleChange} required />
                </div>
                <InputField label={t('modals.addExpense.amount')} id="amount" name="amount" type="number" value={expense.amount} onChange={handleChange} required />
                <div>
                ...
                <div className="flex justify-end gap-3 pt-4 border-t mt-6 dark:border-slate-700">
                    ...
                    <button type="submit" className="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-2 px-4 rounded-lg dark:bg-indigo-600 dark:hover:bg-indigo-700">{t('modals.addExpense.saveButton')}</button>
                </div>
            </form>
        </ModalBase>
    );
