...
import { 
    ...
    getNotifications,
    getPayrollRuns,
    calculatePayslipsForPeriod, // Import the new calculation function
} from '../../firebase/config'; // Moved helper
...
const WalletCard: React.FC = () => {
    ...
    return (
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 flex items-center justify-between">
            <div className="flex items-center gap-4">
            <div className="p-3 rounded-lg bg-indigo-100 dark:bg-indigo-900/50">
                <WalletIcon className="w-8 h-8 text-indigo-500 dark:text-indigo-400" />
            </div>
            <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">{t('partnerDashboard.availableBalance')}</p>
                <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">₦1,250,750</h2>
            </div>
            </div>
            <button className="w-10 h-10 bg-indigo-500 hover:bg-indigo-600 text-white rounded-full flex items-center justify-center transition-colors" aria-label="Top up wallet">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
                <button onClick={() => setActiveModal('createRoute')} className="flex items-center gap-2 text-sm bg-indigo-500 hover:bg-indigo-600 text-white font-semibold px-4 py-2 rounded-lg transition-colors">
                    <MapPinIcon className="w-5 h-5"/> {t('partnerDashboard.createRoute')}
                </button>
                <button onClick={() => setActiveModal('addDriver')} className="flex items-center gap-2 text-sm bg-white border border-gray-300 hover:bg-gray-100 text-gray-700 font-semibold px-4 py-2 rounded-lg transition-colors dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300 dark:hover:bg-slate-600">
                    <UserPlusIcon className="w-5 h-5"/> {t('partnerDashboard.addDriver')}
                </button>
                <button onClick={() => setActiveModal('addVehicle')} className="flex items-center gap-2 text-sm bg-white border border-gray-300 hover:bg-gray-100 text-gray-700 font-semibold px-4 py-2 rounded-lg transition-colors dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300 dark:hover:bg-slate-600">
                    <TruckIcon className="w-5 h-5"/> {t('partnerDashboard.addVehicle')}
                </button>
                <button onClick={() => setActiveModal('addClient')} className="flex items-center gap-2 text-sm bg-white border border-gray-300 hover:bg-gray-100 text-gray-700 font-semibold px-4 py-2 rounded-lg transition-colors dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300 dark:hover:bg-slate-600">
                    <BuildingOffice2Icon className="w-5 h-5"/> {t('partnerDashboard.addClient')}
                </button>
            </div>
        </div>
...
const PartnerDashboard: React.FC<PartnerDashboardProps> = ({ onLogout, role }) => {
  ...
  const handleCompleteRoute = (route: Route) => {
    setRoutes(prev => prev.map(r => r.id === route.id ? { ...r, status: 'Completed', progress: 100 } : r));
  ...
  const handleAssignDriver = (routeId: string, driverId: number, vehicleId: string) => {
  ...
  const handleUpdateDriverPay = (driverId: number, newPayInfo: { baseSalary: number, pensionContributionRate: number, nhfContributionRate: number }) => {
    setDrivers(prev => prev.map(d => d.id === driverId ? { ...d, ...newPayInfo } : d));
    setActiveModal(null);
  };
  ...
  const renderModal = () => {
    ...
    case 'confirmToggleClientStatus':
        ...
        />;
      case 'assignDriver':
        return <AssignDriverModal onClose={() => setActiveModal(null)} route={selectedItem as Route} drivers={drivers.filter(d => d.status === 'Idle')} vehicles={vehicles.filter(v => v.status === 'Active')} onAssign={handleAssignDriver} />;
      case 'viewPOD':
